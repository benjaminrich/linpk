...
An oral dose may have an associated time `lag` and bioavailability (`f`). These
parameters can be added to the dose list argument as necessary:
...
second at 12 hours. We can also specify different amounts for each dose:
...
Doses can also be specified in a `data.frame`:

```{r}
doses <- data.frame(t.dose=c(0, 24*2 + 14), amt=c(100, 50), addl=c(4, 0), ii=24)
doses
...
As a last example, let us consider a slightly more complex scenario in which we
simulate an 150 mg IV dose at time zero, followed by daily subcutaneous 10 mg
doses (first-order absorption) on days 8 to 20 (here we will assume that the
unit of time is days so clearance is in L/day). To do so, it is necessary to
know that we can specify the compartment that a particular dose is administered
...
otherwise. For no particular reason other than to illustrate that it can be
done, we will also specify the dose list as an external `data.frame`:
...
instance. Another limitation is that parameters must be fixed value, they
